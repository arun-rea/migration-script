...
until [ $max -lt $page ];do
    ...
    for i in $(curl "https://api.git.realestate.com.au/orgs/pg-rea-transition/repos?access_token=$token&page=$page&per_page=$repos" | grep '"ssh_url"' | cut -d '"' -f4 ); do
        ...
        # curl -X PUT -H "Accept: application/vnd.github.v3+json" -H "Authorization: token $dest_token" --data "{\"org\":\"propertyguru\",\"team_slug\":\"iproperty\",\"repo\":\"$filename\",\"permission\":\"maintain\",\"owner\":\"propertyguru\"}" https://api.github.com/orgs/propertyguru/teams/iproperty/repos/propertyguru/$filename
